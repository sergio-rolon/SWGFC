        ...
        if(role.equals("operacion")) {
            try (PrintWriter out = response.getWriter()) {
                ...
                try (BufferedReader reader = request.getReader()) {
                    ...
                    Asignaciones asignacionResult = asignacionesService.update(newAsignacion);
                    isDoPut=false;
                    if (asignacionResult != null) {
                    ...
                    response.setStatus(HttpServletResponse.SC_CONFLICT);
                    String errorResponse = "{\"error\": \"Id asignación ingresado ya está asignado, intentar con otro\"}";
                    out.print(errorResponse);
                    out.flush();
                    return;
                } catch (IOException ex) {
